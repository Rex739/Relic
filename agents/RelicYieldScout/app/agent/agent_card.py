...
_NEGOTIATE = AgentSkill(
    id="negotiate",
    name="Request a Venus yield observation",
    description=(
        "Request a read-only comparison of Venus Core Pool supply yields and "
        "liquidity on BSC Testnet. The result is pinned to an observed block and "
        "contains no fabricated token prices or transaction advice."
    ),
    tags=["yield", "venus", "read-only", "bnb-chain"],
...
def build_agent_card() -> AgentCard:
    ...
    return AgentCard(
        name="Relic Yield Scout",
        description=(
            "Compares Venus lending-market supply yields using read-only BSC "
            "Testnet evidence. It cannot move funds or submit DeFi transactions."
        ),
