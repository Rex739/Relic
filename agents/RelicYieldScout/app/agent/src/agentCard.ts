...
/** Public card name from studio.toml (best-effort). */
function agentName(): string {
  let name = "";
  try {
    const cfg = loadStudioToml();
    const project = (cfg.project ?? {}) as Record<string, unknown>;
    name = String(project.display_name ?? project.name ?? "");
  } catch {
...
export function buildAgentCard(
  ...
  return {
    name,
    description:
      "Read-only Venus Core yield observer for BSC Testnet. It ranks supply " +
      "markets from pinned on-chain evidence and cannot move user funds.",
    ...
    skills: opts.commerceSkills === false ? [] : [NEGOTIATE, NOTIFY_FUNDED],
    ...extra,
  };
}
