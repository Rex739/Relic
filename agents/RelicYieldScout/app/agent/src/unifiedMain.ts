...
// Relic Yield Scout observes Venus directly at one pinned BSC Testnet block.
// No LLM can invent market data or invoke a signing operation.
export function buildRunWork(): RunWork {
  return async (_prompt, { abortSignal }) => {
    if (abortSignal?.aborted) throw new Error("yield observation aborted");
    return renderYieldReport(await scanVenusYields(8, abortSignal));
  };
}
