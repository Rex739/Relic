...
| 3 | Durable job state | Idempotency state machine and atomic Postgres repository/migration are implemented. Layer A now has an authenticated HTTP adapter for Relic's durable store; the matching API endpoints and per-job session retrieval remain to be wired before execution is enabled | In progress |
