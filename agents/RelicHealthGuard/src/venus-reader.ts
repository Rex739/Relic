...
export async function verifyHealthGuardDeployment(config: HealthGuardConfig, pool: HealthGuardPoolConfig, client: VenusPublicClient): Promise<void> {
  ...
  const [usdtCode, vTokenCode, comptrollerCode, underlying, comptroller] = await Promise.all([
    client.getCode({ address: pool.usdt }), client.getCode({ address: pool.venusUsdtVToken }), client.getCode({ address: pool.venusComptroller }),
    client.readContract({ address: pool.venusUsdtVToken, abi: vTokenAbi, functionName: "underlying" }),
    client.readContract({ address: pool.venusUsdtVToken, abi: vTokenAbi, functionName: "comptroller" }),
  ]);
  ...
  if (!sameAddress(underlying, pool.usdt)) throw new Error("Health Guard configured vToken does not use configured debt asset");
  if (!sameAddress(comptroller, pool.venusComptroller)) throw new Error("Health Guard configured vToken is bound to a different comptroller");
}
...
export class VenusMainnetHealthReader {
  ...
  public constructor(private readonly config: HealthGuardConfig, private readonly pool: HealthGuardPoolConfig, client?: VenusPublicClient) {
    this.client = client ?? createPublicClient({ chain: bsc, transport: http(config.rpcUrl) });
  }

  verifyDeployment(): Promise<void> {
    return verifyHealthGuardDeployment(this.config, this.pool, this.client);
  }
  ...
  async observation(mandate: HealthGuardMandate): Promise<HealthObservation> {
    ...
    const [assets, oracle, debtSnapshot, reserve] = await Promise.all([
      this.client.readContract({ address: this.pool.venusComptroller, abi: comptrollerAbi, functionName: "getAssetsIn", args: [mandate.borrower], blockNumber }),
      this.client.readContract({ address: this.pool.venusComptroller, abi: comptrollerAbi, functionName: "oracle", blockNumber }),
      this.client.readContract({ address: this.pool.venusUsdtVToken, abi: vTokenAbi, functionName: "getAccountSnapshot", args: [mandate.borrower], blockNumber }),
      this.client.readContract({ address: this.pool.usdt, abi: erc20Abi, functionName: "balanceOf", args: [mandate.rescueWallet], blockNumber }),
    ]);
    ...
    const debtPrice = await this.client.readContract({ address: oracle, abi: oracleAbi, functionName: "getUnderlyingPrice", args: [this.pool.venusUsdtVToken], blockNumber });
    ...
    for (const asset of assets) {
      const [snapshot, market, price] = await Promise.all([
        ...
        this.client.readContract({ address: this.pool.venusComptroller, abi: comptrollerAbi, functionName: "markets", args: [asset], blockNumber }),
  ...
  allowance(owner: Address): Promise<bigint> {
    return this.client.readContract({ address: this.pool.usdt, abi: erc20Abi, functionName: "allowance", args: [owner, this.pool.venusUsdtVToken] });
  }
