...
const NEGOTIATE: AgentSkill = {
  ...
  tags: ["erc8183", "lp-rebalancing", "pancakeswap-v3", "bnb-chain"],
...
const NOTIFY_FUNDED: AgentSkill = {
  ...
  tags: ["erc8183", "lp-rebalancing", "pancakeswap-v3", "delivery", "bnb-chain"],
  inputModes: ["application/json"],
  outputModes: ["application/json"],
};
...
export function buildAgentCard(
  ...
  return {
    name,
    description:
      "Relic LP Range Rebalancer — a deterministic BNB/USDT PancakeSwap V3 " +
      "range-planning seller. It evaluates one position against a buyer-supplied " +
      "price range, capital cap, duration, and one-hour cooldown. " +
      `ERC-8183 seller agent (${name}) — negotiate + notify_funded over A2A.`,
