...
export async function POST(
  ...
  const response = await fetch(
    `${apiUrl()}/v1/mandates/${encodeURIComponent(id)}/altana-session-authorization/confirm`,
    {
      method: "POST",
      cache: "no-store",
      headers: {
  ...
  return Response.json(payload.data ?? { error: payload.error?.message ?? "Could not verify wallet permission" }, { status: response.status });
}
