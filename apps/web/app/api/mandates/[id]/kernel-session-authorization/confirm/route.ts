...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const response = await fetch(
    `${apiUrl()}/v1/mandates/${encodeURIComponent(id)}/kernel-session-authorization/confirm`,
    {
      method: "POST",
      cache: "no-store",
  ...
  return Response.json(payload.data ?? { error: payload.error?.message ?? "Could not store smart-account permission" }, { status: response.status });
}
