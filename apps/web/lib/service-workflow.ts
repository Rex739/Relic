...
const workflows: Record<MarketplaceCategory, ServiceWorkflow> = {
  ...
  "yield-optimisation": {
    ...
    taskDescription: "Supply and withdraw a deliberately small BSC Testnet USDT amount through Venus, within the limits you set.",
    confirmLabel: "Review secure yield permission",
    requirements: [
      {
        name: "capitalCap",
        label: "Maximum supplied TEST_USDT",
        placeholder: "e.g. 1",
        helper: "The most TEST_USDT this session may supply to Venus. Start with a small test amount.",
        required: true,
        type: "number",
        min: 0.000000000000000001,
        step: "any",
      },
      {
        ...
        label: "Test execution amount",
        placeholder: "e.g. 0.1",
        helper: "One tiny supply-and-withdraw run. It can never exceed your supplied cap.",
        required: true,
        type: "number",
        min: 0.000000000000000001,
        step: "any",
      },
      {
        ...
        helper: "A strict total gas cap for this one testnet execution.",
        required: true,
        type: "number",
        min: 0.000000000000000001,
        step: "any",
      },
      {
        ...
        placeholder: "e.g. 2",
        helper: "The buyer-owned session expires automatically after this time.",
        required: true,
        type: "number",
        min: 1,
        max: 168,
    ...
    deliverables: ["Venus supply transaction evidence", "Venus withdrawal transaction evidence", "Reconciled execution receipt and final balance status"],
    permissionSummary: "The agent may use only the prepared BSC Testnet USDT and Venus vToken, under your exact spend, gas, expiry, and one-run limits. You can revoke the session at any time.",
  },
};
